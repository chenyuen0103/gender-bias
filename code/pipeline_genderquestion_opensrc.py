...
columns = ['debias_acronym', 'gender_expression', 'pronoun', 'job','prompt', 'gender_probabilities', 'total_prob']
    ...
    for i, (gender, gender_exp) in enumerate(zip(genders, gender_expressions)):
        ...
        for job in jobs:
            for pronoun in gender_exp:
                ...
                model=model,
                messages=f"{prompt_text}{pronoun}",
                temperature=0,
                max_tokens=0,
                top_p=1,
                frequency_penalty=0,
                ...
                new_row = pd.DataFrame([[debias_acronym, gender_exp, pronoun, job, prompt, gender_probabilities, total_prob]],
                                       columns=columns)
